...
class Zolw(Animal):
    def __init__(self, position, Settings):
        ...
        self.symbol = self.Settings.symbol_zolw
        self.power = self.Settings.power_zolw
        self.initiative = self.Settings.initiative_zolw
        self.color = self.Settings.color_zolw
    ...
    def action(self, objects):
        ...
        if random.randint(0, 3) != 0:
            ...
            if objects[next_point.x][next_point.y].get_symbol() == self.Settings.symbol_zolw:
                self.add(objects, next_point)
                next_point = self.get_next_move()
                self.add(objects, next_point)
            elif objects[next_point.x][next_point.y].power < 5:
                pass
